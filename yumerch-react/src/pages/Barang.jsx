...
export default function Barang() {
  ...
  const submit = async (e) => {
    ...
    const res = editId ? await updateBarang(editId, payload) : await addBarang(payload);
    if (res.ok) { toast(editId ? 'Barang diperbarui.' : 'Barang ditambahkan.'); setShowForm(false); }
    else toast(res.message, 'error');
  };

  const confirmHapus = async () => {
    const res = await deleteBarang(hapus.id_barang);
    toast(res.ok ? 'Barang dihapus.' : res.message, res.ok ? 'success' : 'error');
    setHapus(null);
  };
