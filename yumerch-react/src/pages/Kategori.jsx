...
export default function Kategori() {
  ...
  const submit = async (e) => {
    e.preventDefault();
    const res = editId ? await updateKategori(editId, form) : await addKategori(form);
    if (res.ok) { toast(editId ? 'Kategori diperbarui.' : 'Kategori ditambahkan.'); setShowForm(false); }
    else toast(res.message, 'error');
  };

  const confirmHapus = async () => {
    const res = await deleteKategori(hapus.id_kategori);
